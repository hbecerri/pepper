...
- `trigger_sfs`: Optional, array where the first element is a ROOT file containing the histograms for the trigger scale factors, and the second element gives the histogram(s) to use. This can either be a string (for a single histogram binned in 1st and 2nd lepton pt), an array of strings (when using one histogram per channel) or an array of arrays of strings (where the histograms are split both by channel and by the number of electrons in the barrel in ascending order, so the ee channel will have three elements (`[out, split, in]`), the emu channel two (`[out, in]`), and the mumu channel one).
- `electron_sf`: Optional, array of arrays. The inner ones contain exactly three elements, the first one being a path to a scale factor ROOT file provided by the electron POG, the second one being the name of the histogram to use inside the ROOT file, usually "EGamma_SF2D", the third one is an array specifying the order of the axes in the histogram. Expected axes are "eta" and "pt".
- `muon_sf`: Optional, array of arrays. The inner ones contain exactly three elements, the first one being a path to a scale factor ROOT file provided by the muon POG, the second one being the name of the histogram to use inside the ROOT file, the third one is an array specifying the order of the axes in the histogram. Expected axes are "abseta" and "pt".
