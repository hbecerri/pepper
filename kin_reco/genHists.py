...
class ttbarProcessor(processor.ProcessorABC):
    ...
    def process(self, df):
        ...
        genpart = JaggedCandidateArray.candidatesfromcounts(
            ...
            phi=df["GenPart_phi"],
            mass=np.where(
                (np.abs(df["GenPart_pdgId"]) == 5) &
                (df["GenPart_mass"] == 0), 4.8, df["GenPart_mass"]),
            # mass is only stored for masses greater than 10GeV-
        ...
        genantilep = genpart[((genpart.pdgId == -11) | (genpart.pdgId == -13))
                             & (genpart[genpart.motherIdx].pdgId == 24)
                             & (genpart.statusFlags >> 12 & 1 == 1)]
        genb = genpart[(genpart.pdgId == 5)
                       & (genpart[genpart.motherIdx].pdgId == 6)
                       & (genpart.statusFlags >> 12 & 1 == 1)
                       & (genpart.statusFlags >> 7 & 1 == 1)]
        genantib = genpart[(genpart.pdgId == -5)
                           & (genpart[genpart.motherIdx].pdgId == -6)
                           & (genpart.statusFlags >> 12 & 1 == 1)
                           & (genpart.statusFlags >> 7 & 1 == 1)]
...
provider = CondorProvider(
    channel=LocalChannel(), init_blocks=100, min_blocks=5,
    max_blocks=1000, nodes_per_block=1, parallelism=1,
...
ttbar = (glob.glob("/pnfs/desy.de/cms/tier2/store/mc/RunIIAutumn18NanoAODv5"
                   "/TTTo2L2Nu_TuneCP5_13TeV-powheg-pythia8/NANOAODSIM/"
                   "Nano1June2019_102X_upgrade2018_realistic_v19-v1"
                   "/250000/*.root")
         + glob.glob("/pnfs/desy.de/cms/tier2/store/mc"
...
outfile = uproot.recreate(
    "/nfs/dust/cms/user/stafford/coffea"
    "/desy-ttbarbsm-coffea/data/GenHists.root")
