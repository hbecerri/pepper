...
class ttbarProcessor(processor.ProcessorABC):
    ...
    def process(self, df):
        ...
        genb=genb[lepcuts]
        genb=genb[dilepcuts]
        genb=genb[twogoodjets]
        genb=genb[MET_cut]
        genb=genb[Btagcut]
        genb=genb[METnotalignedwithjets]
        genantib=genpart[(genpart.pdgId==-5)&(genpart[genpart.motherIdx].pdgId==-6)&(genpart.statusFlags>>12&1==1)&(genpart.statusFlags>>7&1==1)]
        genantib=genantib[lepcuts]
        genantib=genantib[dilepcuts]
        genantib=genantib[twogoodjets]
        genantib=genantib[MET_cut]
        genantib=genantib[Btagcut]
        genantib=genantib[METnotalignedwithjets]
        print(bs.shape, genb.shape)
        bangles=bs['p4'].delta_r(genb['p4'].flatten())
        bbarangles=bbars['p4'].delta_r(genantib['p4'].flatten())
        
        genbestbpair=(bangles+bbarangles).argmin()
#        genbestbpair=genbestbpair[Reco_nocomb]
#        bestbpairMlb=bestbpairMlb[Reco_nocomb]
        print(len(genbestbpair.flatten()), len(bestbpairMlb.flatten()))
        mlbgood=(genbestbpair.flatten()==bestbpairMlb.flatten())
        print("mlbgood:", mlbgood.sum()/len(bestbpairMlb.flatten()))
...
#fileset = {'TTbar': ttbar}
fileset = {'TTbar': ["/pnfs/desy.de/cms/tier2/store/mc/RunIIAutumn18NanoAODv5/TTTo2L2Nu_TuneCP5_13TeV-powheg-pythia8/NANOAODSIM/Nano1June2019_102X_upgrade2018_realistic_v19-v1/250000/5239A9C7-9C9D-2B4B-B337-374252B493F0.root"]}
